...
def save_data(path, code_x, visit_lens, codes_y, hf_y, divided, neighbors, visit_intervals):
    save_sparse(os.path.join(path, 'code_x'), code_x)
    np.savez(os.path.join(path, 'visit_lens'), lens=visit_lens)
    #
    np.savez(os.path.join(path, 'visit_intervals'), intervals=visit_intervals)
