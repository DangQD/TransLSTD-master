...
class EHRDataset:
    def __init__(self, data_path, label='m', batch_size=32, shuffle=True, device=torch.device('cpu')):
        super().__init__()
        self.path = data_path
        self.code_x, self.visit_lens, self.y, self.divided, self.neighbors, self.visit_intervals = self._load(label)
    ...
    def _load(self, label):
        ...
        # code_frequences = np.load(os.path.join(self.path, 'code_frequences.npz'))['frequences']
        if label == 'm':
        ...
        return code_x, visit_lens, y, divided, neighbors, visit_intervals
    ...
    def __getitem__(self, index):
        ...
        code_x = torch.from_numpy(self.code_x[slices]).to(device, dtype=torch.float32)
        ...
        # code_frequences = torch.Tensor(self.code_frequences).to(device=device, dtype=torch.float32)
        ...
        return code_x, visit_lens, y, visit_intervals
        # return code_x, visit_lens, divided, y, neighbors, visit_intervals
